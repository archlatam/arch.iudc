...
- **Updates tab** — lists pending repository and AUR updates side by side with current → new versions; *Upgrade all* hands off to Omarchy's official updater (`omarchy-update`) in a floating terminal, and a separate button syncs the package databases (`pacman -Sy`) inside the panel's transaction view.
...
- **Installed tab** — browse native vs foreign packages, jump straight to details, remove packages.
...
- **Cancellable transactions** — a running install (including long AUR builds), database sync, removal or cache cleaning can be stopped from the transaction view: *Cancel* sends SIGTERM to the whole process tree (yay, makepkg and its privileged children), escalating to SIGKILL after 5 s if needed.
- **Details view** — description, version, repository, install reason, dependencies, reverse dependencies and licenses for any package.
...
| `polkit` agent + `pkexec` | yes | Privileged repo and AUR install steps (Omarchy ships one) |
...
```sh
omarchy plugin update arch.iudc
```

After updating, reload the Omarchy shell so the new version takes effect.
...
| Key | Action |
|---|---|
| `Esc` | Close the panel, or leave the finished transaction view |
| `r` | Check for updates |
| `u` | Open Omarchy's updater (when no transaction is running) |
| `1`–`4` | Switch to Updates / Search / Installed / Cache tab |
...
omarchy-shell arch.iudc upgrade   # open Omarchy's updater
omarchy-shell arch.iudc open      # open without toggling
omarchy-shell arch.iudc close     # close
omarchy-shell arch.iudc show      # alias of open
omarchy-shell arch.iudc hide      # alias of close
```
...
- **AUR operations** must run as your regular user (never as root), so `yay` is executed unprivileged. Because yay internally calls `sudo` for the final install step, the plugin ships a tiny PATH shim that intercepts those calls: only `sudo pacman …` is escalated via `pkexec`, so it is authorized by your desktop's standard polkit agent — the exact same mechanism used for repository operations, with no custom dialogs and no password handling of any kind. Any other `sudo` invocation — e.g. one hidden inside a PKGBUILD's prepare/build/package functions — falls through to the real, unmodified sudo, which prompts on the controlling terminal instead of a blank-check escalation. The shim adds no arguments of its own and forwards everything verbatim.
